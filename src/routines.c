...
void OutputWriteLong(unsigned int val) {
#ifdef CALCULATOR
    *(unsigned int *)ice.programPtr = val;
#else
...
void OutputWriteMem(uint8_t mem[]) {
    unsigned int size = strlen((char *) mem);
...
bool IsA2ByteTok(uint8_t tok) {
    ...
    return memchr(All2ByteTokens, tok, sizeof(All2ByteTokens)) != NULL;
}
...
void ProgramPtrToOffsetStack(void) {
    ice.dataOffsetStack[ice.dataOffsetElements++] = (unsigned int *) (ice.programPtr + 1);
...
void ChangeRegValue(unsigned int inValue, unsigned int outValue, uint8_t opcodes[7]) {
...
void LoadRegValue(uint8_t reg2, unsigned int val) {
    if (reg2 == REGISTER_HL) {
        if (reg.HLIsNumber) {
...
void CallRoutine(bool *routineBool, unsigned int *routineAddress, const uint8_t *routineData, uint8_t routineLength) {
...
void WriteIntToDebugProg(unsigned int num) {
    ti_Write(&num, sizeof(unsigned int), 1, debug.dbgPrgm);
}

void WriteWordToDebugProg(unsigned int num) {
    ti_Write(&num, sizeof(uint16_t), 1, debug.dbgPrgm);
}
...
void printButton(unsigned int xPos) {
    gfx_SetColor(0);
